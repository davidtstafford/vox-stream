...
import com.voxstream.core.config.JacksonConfig;
...
/**
 * Tests for PlatformConnectionManager basic lifecycle and auto-reconnect using DummyPlatformConnection failure script.
 */
public class PlatformConnectionManagerTest {
    ...
    private AnnotationConfigApplicationContext baseCtx() {
        ...
        ctx.registerBean(JdbcTemplate.class, () -> {
            ...
            jt.execute("CREATE TABLE IF NOT EXISTS app_config (cfg_key VARCHAR(128) PRIMARY KEY, cfg_value CLOB NULL, updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP)");
            jt.execute("CREATE TABLE IF NOT EXISTS events (id VARCHAR(36) PRIMARY KEY, type VARCHAR(64), source_platform VARCHAR(32), created_at TIMESTAMP, expires_at TIMESTAMP NULL, importance INT, correlation_id VARCHAR(64), payload CLOB NULL)");
            return jt;
        });
        // Added encryption service bean required by JdbcConfigDao constructor
        ctx.registerBean(EncryptionService.class);
        ctx.registerBean(ConfigDao.class, () -> new JdbcConfigDao(ctx.getBean(JdbcTemplate.class), ctx.getBean(EncryptionService.class)));
    ...
    void lifecycleAndReconnect() throws Exception {
        try (AnnotationConfigApplicationContext ctx = baseCtx()) {
            ...
            // Prepare dummy connection scripted failures BEFORE manager start so initial attempt fails
